...
contract HecoPool is Ownable {
    ...
    function getMdxBlockReward(uint256 _lastRewardBlock)
        public
        view
        returns (uint256)
    {
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 mdxReward = blockReward.mul(pool.allocPoint).div(
            totalAllocPoint
        );
        ...
        returns (uint256, uint256)
    {
        PoolInfo storage pool = poolInfo[_pid];
        if (isMultLP(address(pool.lpToken))) {
            (uint256 mdxAmount, uint256 tokenAmount) = pendingMdxAndToken(
                _pid,
                _user
            );
    ...
    function pendingMdxAndToken(uint256 _pid, address _user)
        private
        view
        returns (uint256, uint256)
    {
        ...
        if (user.amount > 0) {
            uint256 TokenPending = IMasterChefHeco(multLpChef).pending(
                poolCorrespond[_pid],
                address(this)
            );
            ...
            uint256 userPending = user
                .amount
                .mul(accMultLpPerShare)
                .div(1e12)
                .sub(user.multLpRewardDebt);
            if (block.number > pool.lastRewardBlock) {
                uint256 blockReward = getMdxBlockReward(pool.lastRewardBlock);
                uint256 mdxReward = blockReward.mul(pool.allocPoint).div(
                    totalAllocPoint
                ...
                accMdxPerShare = accMdxPerShare.add(
                    mdxReward.mul(1e12).div(pool.totalAmount)
                );
                return (
                    user.amount.mul(accMdxPerShare).div(1e12).sub(
                        user.rewardDebt
                    ),
                    userPending
                );
    ...
    function depositMdxAndToken(
        ...
        if (user.amount > 0) {
            ...
            uint256 tokenPending = user
                .amount
                .mul(pool.accMultLpPerShare)
                .div(1e12)
                .sub(user.multLpRewardDebt);
            if (tokenPending > 0) {
        ...
        if (_amount > 0) {
            ...
            if (pool.totalAmount == 0) {
                IMasterChefHeco(multLpChef).deposit(
                    poolCorrespond[_pid],
                    _amount
                );
                ...
                uint256 beforeToken = IERC20(multLpToken).balanceOf(
                    address(this)
                );
                IMasterChefHeco(multLpChef).deposit(
                    poolCorrespond[_pid],
                ...
                uint256 afterToken = IERC20(multLpToken).balanceOf(
                    address(this)
                );
                pool.accMultLpPerShare = pool.accMultLpPerShare.add(
                    afterToken.sub(beforeToken).mul(1e12).div(pool.totalAmount)
